...
batch_size <- 32 # number of sequences to look each time during training
total_epochs <- 15 # number of times we re-use the whole dataset to train model

# set a random seed for reproducibility
set.seed(123)

# select out the column with info on how often it rained
...
trained_model <- model %>% fit(
  ...
  epochs = total_epochs, # how many times we re-use the whole dataset
